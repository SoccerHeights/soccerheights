...
const SeasonPicker = ({seasons,current,onSelect,admin,onStatus,onDelete}) => {
  ...
  const StatusBtns = ({s}) => {
    ...
    return <div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}} onClick={e=>e.stopPropagation()}>
      ...
      {s.status==="active"&&<Btn sz="sm" v="secondary" onClick={()=>onStatus(s.id,"completed")}>Complete</Btn>}
...
export default function App() {
  ...
  if(!role) return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0d1117 0%,#131a24 50%,#0d1117 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'DM Sans',sans-serif"}}>
    ...
    <Card style={{marginBottom:12,textAlign:"left"}}><div style={{fontSize:13,fontWeight:700,color:"#00C896",textTransform:"uppercase",marginBottom:14}}>Admin Login</div><Inp label="Password" type="password" value={pw} onChange={v=>{setPw(v);setErr("");}} ph="Enter admin password"/>{err&&<div style={{color:"#E63946",fontSize:12,marginBottom:8}}>{err}</div>}<Btn onClick={()=>{if(pw===data.adminPw){setRole("admin");setErr("");setTab("seasons");}else setErr("Incorrect password");}} style={{width:"100%"}}>Sign In as Admin</Btn></Card>
    <Card style={{textAlign:"left"}}><div style={{fontSize:13,fontWeight:700,color:"#457B9D",textTransform:"uppercase",marginBottom:14}}>Player Access</div><p style={{color:"#8892a4",fontSize:13,margin:"0 0 14px"}}>View schedules, standings, and league rules.</p><Btn v="secondary" onClick={()=>{setRole("player");setTab("seasons");}} style={{width:"100%"}}>Enter as Player</Btn></Card></div></div>;
  ...
  const GameCard = ({g,admin}) => {
    ...
    return <Card style={{marginBottom:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div style={{fontSize:12,color:"#8892a4",display:"flex",alignItems:"center",gap:6}}><I n="cal" s={14}/>{fmtDate(g.date)} · {g.time}{g.loc?` · ${g.loc}`:""}</div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:16,marginTop:14,fontSize:15,fontWeight:600,color:"#e8ecf4"}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flex:1,justifyContent:"flex-end"}}><span>{ho.name}</span><div style={{width:12,height:12,borderRadius:"50%",background:ho.color}}/></div>
        {g.done?<div style={{padding:"4px 14px",borderRadius:8,background:"rgba(255,255,255,0.06)",fontWeight:700,fontSize:18,fontFamily:"'Bricolage Grotesque',sans-serif",minWidth:60,textAlign:"center"}}>{g.hs} – {g.as}</div>:<div style={{padding:"4px 14px",borderRadius:8,background:"rgba(255,255,255,0.04)",color:"#8892a4",fontSize:13}}>vs</div>}
        <div style={{display:"flex",alignItems:"center",gap:8,flex:1}}><div style={{width:12,height:12,borderRadius:"50%",background:aw.color}}/><span>{aw.name}</span></div>
      </div>
      ...
      {admin&&<div style={{display:"flex",gap:6,marginTop:12,justifyContent:"center",flexWrap:"wrap"}}>
        ...
        {g.done&&<Btn sz="sm" v="secondary" icon="edit" onClick={()=>{setForm({gid:g.id,hs:String(g.hs),as:String(g.as),h:g.h,a:g.a,videoUrl:g.videoUrl||""});setModal("score");}}>Edit Score</Btn>}
        <Btn sz="sm" v="secondary" icon="edit" onClick={()=>{setForm({gid:g.id,h:g.h,a:g.a,date:g.date,time:g.time,loc:g.loc,phase:g.phase||"group"});setModal("editGame");}}>Edit</Btn>
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0d1117 0%,#131a24 50%,#0d1117 100%)",fontFamily:"'DM Sans',sans-serif"}}>
    ...
    <Modal open={modal==="newSeason"} onClose={()=>setModal(null)} title="New Season"><Inp label="Name" value={form.name||""} onChange={v=>setForm({...form,name:v})} ph="e.g., Spring 2026"/><Inp label="Start" type="date" value={form.start||""} onChange={v=>setForm({...form,start:v})}/><Inp label="End" type="date" value={form.end||""} onChange={v=>setForm({...form,end:v})}/><Btn onClick={()=>{if(!form.name)return;upd(d=>({...d,seasons:[...d.seasons,{id:`s${Date.now()}`,name:form.name,status:"planning",start:form.start,end:form.end,teams:[],games:[],groups:["A","B"]}]}));setModal(null);}} disabled={!form.name}>Create</Btn></Modal>

    <Modal open={modal==="addTeam"} onClose={()=>setModal(null)} title="Add Team"><Inp label="Name" value={form.name||""} onChange={v=>setForm({...form,name:v})}/><div style={{marginBottom:14}}><label style={{display:"block",fontSize:12,color:"#8892a4",marginBottom:5,fontWeight:600,textTransform:"uppercase",fontFamily:"'DM Sans',sans-serif"}}>Color</label><input type="color" value={form.color||"#00C896"} onChange={e=>setForm({...form,color:e.target.value})} style={{width:50,height:36,border:"none",borderRadius:8,cursor:"pointer",background:"transparent"}}/></div><Sel label="Group" value={form.group||"A"} onChange={v=>setForm({...form,group:v})} opts={[{v:"A",l:"Group A"},{v:"B",l:"Group B"}]}/><Inp label="Captain Name" value={form.capName||""} onChange={v=>setForm({...form,capName:v})}/><Inp label="Captain Email" value={form.capEmail||""} onChange={v=>setForm({...form,capEmail:v})}/><Btn onClick={()=>{if(!form.name)return;updSeason(s=>({...s,teams:[...s.teams,{id:`t${Date.now()}`,name:form.name,color:form.color||"#00C896",cap:form.capName?{name:form.capName,email:form.capEmail}:null,group:form.group||"A"}]}));setModal(null);}}>Add</Btn></Modal>
    ...
    <Modal open={modal==="addGame"} onClose={()=>setModal(null)} title="Add Game"><Sel label="Home" value={form.h||""} onChange={v=>setForm({...form,h:v})} opts={teamOpts}/><Sel label="Away" value={form.a||""} onChange={v=>setForm({...form,a:v})} opts={teamOpts.filter(o=>o.v!==form.h)}/><Inp label="Date" type="date" value={form.date||""} onChange={v=>setForm({...form,date:v})}/><Inp label="Time" value={form.time||""} onChange={v=>setForm({...form,time:v})} ph="10:00 AM"/><Inp label="Location" value={form.loc||""} onChange={v=>setForm({...form,loc:v})}/><Sel label="Phase" value={form.phase||"group"} onChange={v=>setForm({...form,phase:v})} opts={[{v:"group",l:"Group"},{v:"playoff",l:"Playoff"}]}/><Btn onClick={()=>{if(!form.h||!form.a||!form.date)return;updSeason(s=>({...s,games:[...s.games,{id:`g${Date.now()}`,h:form.h,a:form.a,date:form.date,time:form.time||"",loc:form.loc||"",hs:null,as:null,done:false,phase:form.phase||"group",videoUrl:""}]}));setModal(null);}}>Add</Btn></Modal>

    <Modal open={modal==="score"} onClose={()=>setModal(null)} title="Enter Score">{form.gid&&<><div style={{textAlign:"center",marginBottom:16,fontSize:15,color:"#e8ecf4",fontWeight:600}}>{tm[form.h]?.name} vs {tm[form.a]?.name}</div><div style={{display:"flex",gap:12}}><Inp label={tm[form.h]?.name||"Home"} type="number" value={form.hs??""} onChange={v=>setForm({...form,hs:v})} ph="0"/><Inp label={tm[form.a]?.name||"Away"} type="number" value={form.as??""} onChange={v=>setForm({...form,as:v})} ph="0"/></div><Inp label="Video URL (optional)" value={form.videoUrl||""} onChange={v=>setForm({...form,videoUrl:v})} ph="https://app.veo.co/..."/><Btn onClick={()=>{if(form.hs===""||form.as==="")return;updSeason(s=>({...s,games:s.games.map(g=>g.id===form.gid?{...g,hs:parseInt(form.hs),as:parseInt(form.as),done:true,videoUrl:form.videoUrl||""}:g)}));setModal(null);}}>Save</Btn></>}</Modal>

    <Modal open={modal==="editGame"} onClose={()=>setModal(null)} title="Edit Game"><Sel label="Home" value={form.h||""} onChange={v=>setForm({...form,h:v})} opts={teamOpts}/><Sel label="Away" value={form.a||""} onChange={v=>setForm({...form,a:v})} opts={teamOpts}/><Inp label="Date" type="date" value={form.date||""} onChange={v=>setForm({...form,date:v})}/><Inp label="Time" value={form.time||""} onChange={v=>setForm({...form,time:v})}/><Inp label="Location" value={form.loc||""} onChange={v=>setForm({...form,loc:v})}/><Sel label="Phase" value={form.phase||"group"} onChange={v=>setForm({...form,phase:v})} opts={[{v:"group",l:"Group"},{v:"playoff",l:"Playoff"}]}/><Btn onClick={()=>{updSeason(s=>({...s,games:s.games.map(g=>g.id===form.gid?{...g,h:form.h,a:form.a,date:form.date,time:form.time,loc:form.loc,phase:form.phase}:g)}));setModal(null);}}>Save</Btn></Modal>

    <Modal open={modal==="video"} onClose={()=>setModal(null)} title="Game Video Link"><Inp label="Video URL" value={form.videoUrl||""} onChange={v=>setForm({...form,videoUrl:v})} ph="https://app.veo.co/..."/><Btn onClick={()=>{updSeason(s=>({...s,games:s.games.map(g=>g.id===form.gid?{...g,videoUrl:form.videoUrl||""}:g)}));setModal(null);}}>Save</Btn></Modal>
