...
export default function App() {
  ...
  const GameCard = ({g,admin}) => {
    ...
    return <Card style={{marginBottom:10}}>
      ...
      {admin&&<div style={{display:"flex",gap:6,marginTop:12,justifyContent:"center",flexWrap:"wrap"}}>
        ...
        <Btn sz="sm" v="secondary" icon="edit" onClick={()=>{setForm({gid:g.id,h:g.h,a:g.a,date:g.date,time:g.time,loc:g.loc,duration:g.duration?String(g.duration):"",phase:g.phase||"group"});setModal("editGame");}}>Edit</Btn>
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0d1117 0%,#131a24 50%,#0d1117 100%)",fontFamily:"'DM Sans',sans-serif"}}>
    ...
    <Modal open={modal==="addGame"} onClose={()=>setModal(null)} title="Add Game"><Sel label="Home" value={form.h||""} onChange={v=>setForm({...form,h:v})} opts={teamOpts}/><Sel label="Away" value={form.a||""} onChange={v=>setForm({...form,a:v})} opts={teamOpts.filter(o=>o.v!==form.h)}/><Inp label="Date" type="date" value={form.date||""} onChange={v=>setForm({...form,date:v})}/><Inp label="Time" value={form.time||""} onChange={v=>setForm({...form,time:v})} ph="10:00 AM"/><Sel label="Field" value={form.loc||""} onChange={v=>setForm({...form,loc:v})} opts={FIELDS.map(f=>({v:f,l:f}))}/><Inp label="Duration (minutes)" type="number" value={form.duration||""} onChange={v=>setForm({...form,duration:v})} ph="70"/><Sel label="Phase" value={form.phase||"group"} onChange={v=>setForm({...form,phase:v})} opts={[{v:"group",l:"Group"},{v:"playoff",l:"Playoff"}]}/><Btn onClick={()=>{if(!form.h||!form.a||!form.date)return;updSeason(s=>({...s,games:[...s.games,{id:`g${Date.now()}`,h:form.h,a:form.a,date:form.date,time:form.time||"",loc:form.loc||"",duration:form.duration?parseInt(form.duration):null,hs:null,as:null,done:false,phase:form.phase||"group",videoUrl:""}]}));setModal(null);}}>Add</Btn></Modal>
    ...
    <Modal open={modal==="editGame"} onClose={()=>setModal(null)} title="Edit Game"><Sel label="Home" value={form.h||""} onChange={v=>setForm({...form,h:v})} opts={teamOpts}/><Sel label="Away" value={form.a||""} onChange={v=>setForm({...form,a:v})} opts={teamOpts}/><Inp label="Date" type="date" value={form.date||""} onChange={v=>setForm({...form,date:v})}/><Inp label="Time" value={form.time||""} onChange={v=>setForm({...form,time:v})}/><Sel label="Field" value={form.loc||""} onChange={v=>setForm({...form,loc:v})} opts={FIELDS.map(f=>({v:f,l:f}))}/><Inp label="Duration (minutes)" type="number" value={form.duration||""} onChange={v=>setForm({...form,duration:v})} ph="70"/><Sel label="Phase" value={form.phase||"group"} onChange={v=>setForm({...form,phase:v})} opts={[{v:"group",l:"Group"},{v:"playoff",l:"Playoff"}]}/><Btn onClick={()=>{const sy=window.scrollY;updSeason(s=>({...s,games:s.games.map(g=>g.id===form.gid?{...g,h:form.h,a:form.a,date:form.date,time:form.time,loc:form.loc,duration:form.duration?parseInt(form.duration):null,phase:form.phase}:g)}));setModal(null);setTimeout(()=>window.scrollTo(0,sy),100);}}>Save</Btn></Modal>
