...
export default function App() {
  ...
  const getWeekend=(dateStr)=>{if(!dateStr)return"";const d=new Date(dateStr+"T12:00:00");if(isNaN(d))return"";const day=d.getDay();let sat;if(day===0){sat=new Date(d);sat.setDate(d.getDate()-1);}else{sat=new Date(d);sat.setDate(d.getDate()+(6-day));}const sun=new Date(sat);sun.setDate(sat.getDate()+1);return sat.toISOString().split("T")[0]+"|"+sun.toISOString().split("T")[0];};
  ...
  if(!role) return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0d1117 0%,#131a24 50%,#0d1117 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'DM Sans',sans-serif"}}>
    ...
    {data.maintenance?<Card style={{marginBottom:12,textAlign:"center",border:"1px solid rgba(255,179,0,0.3)",background:"rgba(255,179,0,0.06)"}}><div style={{fontSize:28,marginBottom:10}}>🚧</div><div style={{fontSize:15,fontWeight:700,color:"#FFB300",marginBottom:8}}>Temporarily Unavailable</div><p style={{color:"#8892a4",fontSize:13,margin:0}}>{data.maintenanceMsg||"We're making updates. Check back shortly!"}</p></Card>
  ...
  const GameCard = ({g,admin}) => {
    ...
    const multiTeams=!g.done&&!teamFilter?[hCount>1?ho.name:null,aCount>1?aw.name:null].filter(Boolean):[];
    return <Card style={{marginBottom:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <div style={{fontSize:12,color:"#8892a4",display:"flex",alignItems:"center",gap:6}}><I n="cal" s={14}/>{fmtDate(g.date)} · {g.time}</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{multiGame&&<Badge c="#03A9F4">⚡ {teamFilter?`2 games`:multiTeams.join(" & ")+` 2 games`}</Badge>}{g.phase==="playoff"&&<Badge c="#FFB300">Playoff</Badge>}{g.done?<Badge c="#00C896">Final</Badge>:<Badge c="#F4A261">Upcoming</Badge>}</div>
      </div>
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0d1117 0%,#131a24 50%,#0d1117 100%)",fontFamily:"'DM Sans',sans-serif"}}>
    ...
    <div style={{padding:20,maxWidth:900,margin:"0 auto"}}>
      ...
      {tab==="schedule"&&season&&<div>
        ...
        {isAdmin&&<><ExcelUploadGames season={season} onImport={(gs,es)=>{updSeason(s=>({...s,games:[...s.games,...gs]}));flash(es.length?`Imported ${gs.length} games. Issues:\n${es.slice(0,10).join("\n")}${es.length>10?"\n...and "+(es.length-10)+" more":""}`:` Imported ${gs.length} games!`);}}/><div style={{display:"flex",justifyContent:"flex-end",gap:8,marginBottom:16}}>{season.games.length>0&&<Btn icon="trash" sz="sm" v="danger" onClick={()=>{if(window.confirm(`Delete all ${season.games.length} games from ${season.name}?`)){if(window.confirm("Are you sure? This cannot be undone.")){updSeason(s=>({...s,games:[]}));flash("All games deleted.");}}}}>Delete All Games</Btn>}<Btn icon="plus" sz="sm" onClick={()=>{setForm({h:"",a:"",date:"",time:"",loc:"",phase:"group"});setModal("addGame");}}>Add Game</Btn></div></>}
        {(()=>{
      ...
      {tab==="settings"&&isAdmin&&<div>
        ...
        <Card style={{marginTop:16}}><div style={{fontWeight:700,color:"#e8ecf4",marginBottom:14}}>Maintenance Mode</div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}><input type="checkbox" id="maint-toggle" checked={!!data.maintenance} onChange={e=>upd(d=>({...d,maintenance:e.target.checked}))} style={{width:20,height:20,accentColor:"#FFB300",cursor:"pointer"}}/><label htmlFor="maint-toggle" style={{color:data.maintenance?"#FFB300":"#8892a4",fontSize:14,fontWeight:600,cursor:"pointer"}}>{data.maintenance?"🚧 Site is DOWN for players":"Site is live"}</label></div><Inp label="Message (optional)" value={data.maintenanceMsg||""} onChange={v=>upd(d=>({...d,maintenanceMsg:v}))} ph="We're making updates. Check back shortly!"/></Card>
